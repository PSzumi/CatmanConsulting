...
export function ShatteredWord({
  ...
  const playAnimation = useCallback(() => {
    ...
    blocks.forEach((block, index) => {
      ...
      // Force 3D layer creation to prevent browser culling of distant elements
      gsap.set(block, {
        x: data.startX,
        y: data.startY,
        z: 0.01, // Small z value forces 3D rendering context
  ...
  // Play animation only once when first becoming visible
  useEffect(() => {
    if (isVisible && isReady && !hasPlayedOnce.current) {
      playAnimation();
    }
  ...
  return (
    <span
      ...
      style={{
        overflow: "visible",
        zIndex: 50,
        // Force browser to render off-screen content
        contain: "none",
      }}
    >
      {/* Invisible element to force browser to allocate render space for far shards */}
      <span
        className="absolute pointer-events-none"
        style={{
          top: -scatterRange * 2,
          left: -scatterRange * 2,
          right: -scatterRange * 2,
          bottom: -scatterRange * 2,
          visibility: "hidden",
        }}
        aria-hidden="true"
      />
      ...
      {letters.map((letter, letterIndex) => (
        ...
        >
          ...
          <span className={letterClassName} style={{ visibility: "hidden", display: "inline-block" }}>
            {letter}
          </span>
